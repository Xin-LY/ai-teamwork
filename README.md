...
| 640×640 | 蒸馏后 (yolov8n) | **0.808** | 3.0M | ~21 |
| 640×640 | 剪枝 0.5 (yolov8n) | 0.804 | 1.06M | 34.96 |
| 416×416 | 蒸馏后 (yolov8n) | **0.812** | 3.0M | ~27 |
| 416×416 | 剪枝 0.5 (yolov8n) | 0.783 | 1.06M | 50.94 |

**核心结论**：在 GPU 加速下完成蒸馏训练，416×416 蒸馏模型 mAP 达 0.812，640×640 蒸馏模型 mAP 达 0.808。蒸馏后模型参数量保持 3.0M 不变，验证了知识蒸馏在工业缺陷检测中的可行性。剪枝后模型参数量降低 64.8%，FPS 最高提升 71.8%，是边缘部署的有效手段。
...
- Python 3.8 或更高版本
- 建议 8GB 以上内存
- 推荐 NVIDIA GPU（本项目使用 RTX 4050 Laptop GPU）
...
#### 4. 安装 PyTorch（GPU 版，适配 RTX 4050）
bash
...
│
├── models/                           # 模型文件
│   ├── baseline_416.pt               # 416 基线模型
│   └── baseline_640.pt               # 640 基线模型
│
├── train_teacher.py                  # 教师模型训练脚本
├── train_distill_416.py              # 416 蒸馏训练脚本
├── train_distill_640.py              # 640 蒸馏训练脚本
├── prune.py                          # 剪枝脚本
└── README.md                         # 本文件
...
model.train(data='NEU-DET/data.yaml', imgsz=416, epochs=50, device=0)

#### 4. 知识蒸馏训练
bash
# 416 分辨率蒸馏
python train_distill_416.py

# 640 分辨率蒸馏
python train_distill_640.py
...
bash
# 评估任意模型
yolo val model=runs/detect/train-2/weights/best.pt data=NEU-DET/data.yaml imgsz=416 plots=False
实验结果汇总
...
Distilled	416×416	0.812	0.482	1.71	3.01	~27
Distilled	640×640	0.808	0.473	4.05	3.01	~21
...
知识蒸馏有效：416×416 下蒸馏后 mAP 达 0.812，640×640 下达 0.808，参数量保持 3.0M 不变，验证了蒸馏在轻量化模型优化中的可行性。

剪枝显著加速：0.5 剪枝使参数量降低 64.8%，FPS 最高提升 71.8%，是边缘部署的最有效手段。

综合最优：精度优先选 640×640 基线或蒸馏模型；速度优先选 416×416 剪枝 0.5；平衡方案选 640×640 剪枝 0.5。
